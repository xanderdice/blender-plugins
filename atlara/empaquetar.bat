...
echo.
echo   Atlara - generador del paquete
echo   ------------------------------
echo.
...
if defined BL (
    ...
    "!BL!" --background --factory-startup --python "pruebas\prueba.py" > "%TEMP%\atlara-pruebas.txt" 2>&1
    if errorlevel 1 (
        echo         FALLAN. Mira %TEMP%\atlara-pruebas.txt
        goto :fin
    )
    for /f "tokens=*" %%L in ('findstr /c:"comprobaciones pasan" "%TEMP%\atlara-pruebas.txt"') do echo         %%L
) else (
